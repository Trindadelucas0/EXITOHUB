...
- Em **Consulta** e **Divergências** da Egaplast, o mesmo filtro de segmento agrupa o cadastro; com `TRIBUTACAO NCM EGAPLAST` a conferência é NCM na base (CEST se o cadastro tiver CEST). A lista mostra SP; a ficha e **Como dar entrada** têm 27 linhas (UF, cadastro do cliente, como deve ficar = IVA da regra CST+IVA da EXITO SIGNATÁRIO; se essa regra não tiver mapa, o IVA do cadastro). Cadastro sem IVA, ou IVA `0`, = **NADA INFORMADO** e essa UF não gera divergência. Zero na regra contra fator do cliente continua divergência. Vermelho no IVA só contra a regra da **mesma origem**. SKU com SIT+IVA diferente da EXITO da mesma origem vai para **Divergências**. No cadastro atual, `10100` e `10200` (SP `1.9424`) divergem em DF e PR (MA, PI, SE e TO estão `0` no cadastro e não divergem); SP fica dentro de 0,05. `10255` tem SIT `10` e diverge em AL, DF e PR. A aba entra pelo cabeçalho: cadastro largo é o lote; relatório só completa IVA vazio; outra aba é ignorada. NCM só na regra CST+IVA usa CST+IVA; NCM em nenhuma base e sem IVA no cadastro: o errado é o NCM.
