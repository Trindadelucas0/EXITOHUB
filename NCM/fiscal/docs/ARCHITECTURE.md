...
Python **não** sobe como API. Roda em `tools/extract_rules.py`: ODS padrão (`data/ncm-atualizado.ods`) — aba `BAIFER` → `data/base-baifer.json`; aba `LOJA` → `data/base-loja.json`. A Unica sai de `prisma/extract-unica.ts` → `data/base-unica.json`. A Egaplast CST+IVA sai de `prisma/extract-egaplast.ts` (planilha EXITO SIGNATÁRIO) → `data/base-egaplast.json`. A planilha `TRIBUTACAO NCM EGAPLAST.xlsx` entra pela tela Base fiscal (layout NCM/CEST/UF). O seed grava cada JSON na empresa correspondente. A aba `Planilha_Classes_Fiscais` não entra no seed.
