...
| Tabela | Papel |
| --- | --- |
| `companies` | Empresa (BAIFER, Loja, Unica e Egaplast no seed) |
...
| `fiscal_ncm_rules` | Uma linha por NCM + situação (`company_id, ncm, situacao_codigo` unique). Na Unica: `cest`, `ipi`, `abreviacao`, `reducao`, `uf_tributacao` (DF/GO/MG). Na Egaplast CST+IVA: `iva_por_uf` (27 UFs) |
| `products` | Cadastro importado (sempre ligado a um lote); `audit_status`, `treated_at`; Unica: `abreviacao` do CSV; Egaplast: `origem`, `iva_por_uf` |
...
Empresa BAIFER + Loja + Unica + Egaplast + admin + consulta + regras + administrador do escritório. **Não apaga lotes.**  
